# Agent Instructions — DDAT (Document-Driven AI Tools)

This project follows DDAT: documents in `Docs/` are the source of truth. Code implements what the documents specify — not the other way around. Full methodology: `Docs/00_ddat/metodologia.md` and `Docs/00_ddat/regras_ddat.md`.

## Before writing any code

1. Read `Docs/00_ddat/metodologia.md` and `Docs/00_ddat/regras_ddat.md` if you haven't in this session — they define the workflow this file assumes.
...
4. Find or create the relevant session under `Docs/05_sessions/` (`ddat session create "<nome>"`) and the block describing the unit of work (`ddat block create "<nome>" --session <sessao>`). Read the block fully before touching code — it defines scope, files involved, and acceptance criteria.
...
- Generate the execution prompt for a block with `ddat prompt create --block <bloco> --session <sessao>` when handing work to an agent.
...
- Register every pendency you find using the P1–P4 scale (`Docs/00_ddat/metodologia.md`, section 12) — never describe unresolved work only in free-form prose.
- When a block is done, generate its closure feedback with `ddat feedback create --block <bloco> --session <sessao>` and fill it in completely — what was built, what changed, what's still pending.
...
- Run `ddat validate` to check structural compliance and `ddat audit` to check for orphaned blocks/prompts/feedbacks before considering a session closed.
