...
_Comandos/testes que precisam passar antes do bloco ser considerado concluído (ex.: `ddae-engine validate`, suíte de testes, lint)._
...
_Pendências que já se sabe, antes de implementar, que provavelmente vão sobrar (classificadas P1–P4 conforme `Docs/00_ddae_engine/metodologia.md`, seção 12)._

- _..._

## 16. Feedback Obrigatório

_Lembrete: ao final deste bloco, gerar e preencher o feedback via `ddae-engine feedback create --block bloco_{{BLOCK_NUMBER}}_{{BLOCK_SLUG}} --session session_{{SESSION_NUMBER}}_{{SESSION_SLUG}}`. Sem feedback preenchido, o bloco não está concluído._
