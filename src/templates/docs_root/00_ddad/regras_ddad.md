...
5. Todo bloco termina com feedback (`ddad feedback create`) preenchido — não apenas gerado.
...
7. Pendências encontradas durante a execução são registradas com prioridade P1/P2/P3/P4 (ver `Docs/00_ddad/metodologia.md`, seção 12) — nunca descritas apenas em prosa livre sem classificação.
8. Nomes de arquivos e pastas gerados pela DDAD usam `snake_case`, sem acentos e sem espaços. Use `ddad validate` para checar isso automaticamente.
...
- Use `ddad audit` periodicamente, não apenas ao fechar uma sessão, para flagrar blocos sem prompt/feedback correspondente o mais cedo possível.
- Escreva critérios de aceite verificáveis (algo que se possa marcar como feito/não feito), não aspiracionais.

## 3. Anti-padrões

- **Implementar primeiro, documentar depois "se der tempo".** Na DDAD a ordem é inversa: o documento existe antes da implementação justamente para definir o que vai ser construído.
...
- **Sessões ou blocos "fantasmas".** Um bloco sem prompt ou sem feedback correspondente (detectável via `ddad audit`) indica trabalho que começou mas não foi encerrado corretamente.
...
- **Sobrescrever sem `--force`.** Se um comando da DDAD reporta "already exists", isso é um sinal para revisar o conteúdo existente antes de decidir sobrescrever, não para adicionar `--force` por reflexo.
