...
// The 10 base sessions stamped out by `ddad init`. Not a limit — `ddad session
...
export function scaffoldSession(destDir, { number, title }, { force, created, skipped }) {
  copyDir(SESSION_TEMPLATE_DIR, destDir, {
    force,
    created,
    skipped,
    transform: (content) => renderTemplate(content, { SESSION_NUMBER: number, SESSION_TITLE: title }),
  });
}
