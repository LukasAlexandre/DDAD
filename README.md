...
your-project/
├── Docs/
│   ├── 00_ddat/                # methodology, rules, folder schema, glossary
...
> The 10 sessions above are a **base starting point, not a limit**. Create as many additional sessions as your project needs with `ddat session create`.
...
node bin/ddat.js validate --dir ./my-project
node bin/ddat.js audit --dir ./my-project
```
...
```bash
npm pack
npm install /path/to/ddat-<version>.tgz   # from another project
...
```
ddat init [--dir <path>] [--force]
ddat session create "<name>" [--dir <path>] [--force]
ddat block create "<name>" --session <session> [--dir <path>] [--force]
ddat prompt create --block <block> --session <session> [--dir <path>] [--force]
ddat feedback create --block <block> --session <session> [--dir <path>] [--force]
ddat validate [--dir <path>]
ddat audit [--dir <path>]
ddat --help
ddat --version
```
...
2. **Work session by session, block by block.** Each unit of work is a block inside a session (`ddat block create`), with a generated prompt (`ddat prompt create`) and a closing feedback doc (`ddat feedback create`).
...
`{{NEXT_BLOCK}}` is reserved but intentionally not auto-rendered — there's no reliable way to know the next block's name when the current one is created. Fill it in by hand when you know it; see `Docs/00_ddat/glossario.md`.

## The full loop

DDAT is documentation + prompts + implementation + feedback + audit + validation — not just a folder generator. A block isn't done when the code works; it's done when:
...
5. `ddat validate` and `ddat audit` pass against the current `Docs/` structure.
